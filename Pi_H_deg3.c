	...
	if(d>=3){
		...
		for(int i3=i2;i3<n;i3++)
		{
			...
			if(index<N){
				...
				fq_mul(y2,y1,fq_mat_entry(x,i1,0),Fp);
				fq_mul(y3,y2,fq_mat_entry(x,i2,0),Fp);
				fq_mul(y4,y3,fq_mat_entry(x,i3,0),Fp);
				fq_add(y,y,y4,Fp);

				if(i1==i3){
					fq_mul_ui(temp,y3,3,Fp);
					fq_add(fq_mat_entry(sigma,i1,0),fq_mat_entry(sigma,i1,0),temp,Fp);
				}else if(i1==i2){
					fq_mul_ui(temp,y2,2,Fp);
					fq_mul(temp,temp,fq_mat_entry(x,i3,0),Fp);
					fq_add(fq_mat_entry(sigma,i1,0),fq_mat_entry(sigma,i1,0),temp,Fp);

					fq_add(fq_mat_entry(sigma,i3,0),fq_mat_entry(sigma,i3,0),y3,Fp);	
				}else if(i2==i3){
					fq_mul(temp,y1,fq_mat_entry(x,i2,0),Fp);
					fq_mul(temp,temp,fq_mat_entry(x,i2,0),Fp);
					fq_add(fq_mat_entry(sigma,i1,0),fq_mat_entry(sigma,i1,0),temp,Fp);

					fq_mul_ui(temp,y2,2,Fp);
					fq_mul(temp,temp,fq_mat_entry(x,i2,0),Fp);
					fq_add(fq_mat_entry(sigma,i2,0),fq_mat_entry(sigma,i2,0),temp,Fp);
				}else{
					fq_mul(temp,y1,fq_mat_entry(x,i2,0),Fp);
					fq_mul(temp,temp,fq_mat_entry(x,i3,0),Fp);
					fq_add(fq_mat_entry(sigma,i1,0),fq_mat_entry(sigma,i1,0),temp,Fp);

					fq_mul(temp,y2,fq_mat_entry(x,i3,0),Fp);
					fq_add(fq_mat_entry(sigma,i2,0),fq_mat_entry(sigma,i2,0),temp,Fp);
					fq_add(fq_mat_entry(sigma,i3,0),fq_mat_entry(sigma,i3,0),y3,Fp);
				}
			}
